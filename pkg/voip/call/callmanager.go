...
type CallManager struct {
	...
	sctpRelay   *transport.SctpRelayManager
...
// SetProxy routes this call's media through the instance proxy. Only SOCKS5
// can carry UDP; with an HTTP proxy the call still rings (signaling rides the
// whatsmeow websocket) but audio cannot flow. Call before placing the call.
func (m *CallManager) SetProxy(proxy *transport.ProxyConfig) {
	if m.sctpRelay != nil {
		m.sctpRelay.SetProxy(proxy)
	}
}
