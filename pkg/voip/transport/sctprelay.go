...
type SctpRelayManager struct {
	...
	// api carries the proxy-aware network stack; nil means the pion default.
	api *webrtc.API
}

// SetProxy configures the proxy that call media should traverse. Must be called
// before the first relay connection is opened.
func (m *SctpRelayManager) SetProxy(proxy *ProxyConfig) {
	api, err := NewProxyAPI(proxy, m.log)
	if err != nil {
		m.log.Error("voip: failed to build proxy-aware webrtc api, using direct", "err", err)
		return
	}
	m.mu.Lock()
	m.api = api
...
// newPeerConnection creates a peer connection on the configured stack.
func (m *SctpRelayManager) newPeerConnection() (*webrtc.PeerConnection, error) {
	m.mu.Lock()
	api := m.api
	m.mu.Unlock()
